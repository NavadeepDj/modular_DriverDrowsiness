...
class FaceDetector:
    ...
    def draw_eye_contours(self, frame, left_eye, right_eye, color=(0, 255, 255), thickness=2):
        """
        Draw a single continuous boundary line around each eye using the
        6-point landmarks we already extract for EAR.

        Args:
            frame: BGR image frame (in-place drawing)
            left_eye: list of 6 (x, y) points for left eye
            right_eye: list of 6 (x, y) points for right eye
            color: BGR tuple for contour color
            thickness: line thickness
        """
        for eye in (left_eye, right_eye):
            if len(eye) == 6:
                pts = np.array(eye, dtype=np.int32).reshape((-1, 1, 2))
                # Draw a closed polyline: this is a single continuous boundary, no dots.
